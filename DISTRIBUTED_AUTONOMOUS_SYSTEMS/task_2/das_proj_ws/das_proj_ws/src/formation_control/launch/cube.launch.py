...
D_M = np.sqrt(3)*L
D_m = np.sqrt(2)*L
# distances = [[0,     L,     D_m,    L,	L,     D_m,      D_M,   D_m],
# 			[L,     0,      L,    D_m,	D_m,     L,      D_m,   D_M],
# 			[D_m,     L,      0,    L,	D_M,     D_m,      L,   D_m],
# 			[L,     D_m,      L,    0,	D_m,     D_M,      D_m,   L],	
# 			[L,     D_m,      D_M,    D_m,	0,     L,      D_m,   L],
# 			[D_m,     L,      D_m,    D_M,	L,     0,      L,   D_m],
# 			[D_M,     D_m,      L,    D_m,	D_m,     L,      0,   L],
# 			[D_m,     D_M,      D_m,    L,	L,     D_m,      L,   0]]

distances = [[0,     L,     D_m,    0,	0,     D_m,      D_M,   D_m],
			[L,     0,      0,    D_m,	D_m,     0,      D_m,   D_M],
			[D_m,     0,      0,    0,	D_M,     D_m,      0,   D_m],
			[0,     D_m,      0,    0,	D_m,     D_M,      D_m,   0],	
			[0,     D_m,      D_M,    D_m,	0,     L,      D_m,   0],
			[D_m,     0,      D_m,    D_M,	L,     0,      L,   D_m],
			[D_M,     D_m,      0,    D_m,	D_m,     L,      0,   0],
			[D_m,     D_M,      D_m,    0,	0,     D_m,      0,   0]]

planar_formation = False # we want to obtain the formation on a given plane
print('\nDistance Matrix:\n')
pprint(distances)
...
def generate_launch_description():
    ...
    for i in range(N):
        ...
        N_tot_i = np.nonzero(Helping_matrix[:][i])[0].tolist() # Indeces of non zero elements (all agents besides itself)
        launch_description.append(
            Node(
