...
while 1: #Create a catomial connected graph
...
plt.figure()
plt.title(f'Consensus on weights and biases\n(final achieved accuracy = {accuracy})')
plt.xlabel("batches")
plt.ylabel("log(sum of distances)")

for agent in range(N_AGENTS):
  plt.plot(np.arange(MAXITERS*BATCHES_x_AGENT), np.log(distances[agent].flatten()), linestyle="-", color = colors[agent])

plt.figure()
plt.title(f'Estimation of the gradient norm \n(final achieved accuracy = {accuracy})')
plt.xlabel("batches")
plt.ylabel("log(norm)")

for agent in range(N_AGENTS):
  plt.plot(np.arange(MAXITERS*BATCHES_x_AGENT), np.log(grad_norm_estimation[agent].flatten()), linestyle="-", color = colors[agent])
...
for agent in range(N_AGENTS):
  plt.plot(np.arange(MAXITERS*BATCHES_x_AGENT), np.log(JJ[agent].flatten()), linestyle="-", color = colors[agent])
plt.show()
