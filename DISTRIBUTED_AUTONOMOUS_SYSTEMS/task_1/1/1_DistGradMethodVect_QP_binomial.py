...
p=0.5
while 1:
	graph = nx.binomial_graph(NN, p) # this function takes as input the number of nodes to connect to a centra node, if NN = 10 then we should give as input 9
...
for kk in range (MAXITERS-1):
	...
	for ii in range (NN):
		...
		_, grad_fii = quadratic_fi(VV[:, ii, kk+1].reshape((ns, 1)), QQ[:, :, ii], RR[:, ii].reshape((ns, 1))) # _, ignores the first output

		dXX = - stepsize*grad_fii
...
######### plots ##########
print('xopt: {} \nfopt: {}\n'.format(xopt.squeeze(),fopt.squeeze()))

for ii in range(NN):
	_, FF_grad = quadratic_fi(XX[:, ii, -1].reshape((ns,1)), QQ[:,:,ii], RR[:,ii].reshape((ns,1))) 
	print('Agent: {}, last iter XX: {}, last iter FF: {}, last iter FFgrad: {}\n'.format(ii, XX[:,ii,-1].squeeze(), FF_temp[-1], FF_grad.squeeze()))
