...
np.save('xx_opt.npy', xxt[:,:,max_iters_newt-1])
np.save('uu_opt.npy', uut[:,:,max_iters_newt-1])
...
if car_animation:
    ...
    xx = np.zeros((ns,TT))
    xx[:, 0] = np.array([0, 0, - np.pi/2, 0.1, 0, 0])
    uu = np.zeros((ni,TT-1))
    uu_msms = np.zeros((3,TT-1))
    coefficient = .0314
    for tt in range(TT-1):
 
        # xx_opt[1, tt] = - xx_opt[1, tt]
        # xx_opt[2, tt] = - xx_opt[2, tt]
        # xx_opt[4, tt] = - xx_opt[4, tt]
        # xx_opt[5, tt] = - xx_opt[5, tt]
        # uu_opt[0] = - uu_opt[0]
